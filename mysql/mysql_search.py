#!/usr/bin/env python3
# Directly adapted from https://github.com/averagesecurityguy/scripts/blob/master/database/mysql_search.py
# All credits goes to averagesecurityguy, Thanks !
...
parser.add_argument('-H', help='Target host', action='store', dest='host', required=True)
parser.add_argument('-P', help='Target port', action='store', dest='port', required=True)
parser.add_argument('-u', help='Username', action='store', dest='user', required=True)
parser.add_argument('-p', help='Password', action='store', dest='pwd', required=True)

args = parser.parse_args()

print('[*] Searching {0} on port {1}'.format(args.host, int(args.port)))
search_db(args.host, args.user, args.pwd, int(args.port))
